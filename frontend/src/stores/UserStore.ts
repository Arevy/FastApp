...
class UserStore {
    ...
    constructor(rootStore: RootStore, apolloClient: ApolloClient<NormalizedCacheObject>) {
        ...
        try {
            ...
            this.queryObservable.subscribe({
                ...
                error: (error) => {
                    // Error handling
                    this.isLoading = false;
                    this.error = error instanceof Error ? error : new Error('An unknown error occurred on subscribe');
                },
            });
        } catch (error) {
